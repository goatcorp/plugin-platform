...
<div class="info">
	<p>By <a href={`/user/${data.preset.author}`} class="link">{data.authorName}</a></p>
	<p>Created {data.preset.created.toLocaleDateString()}</p>
	<p>Last updated {data.preset.updated.toLocaleDateString()}</p>
	<p>{data.presetStats.views} views</p>
	{#if isFavorite != null}
		<button class="btn btn-accent" on:click={toggleFavorite}
			>{isFavorite ? 'Remove from favorites' : 'Add to favorites'}</button
		>
	{/if}

	{#if user?.profile?.id === data.preset.author}
		<a href={`/preset/${data.preset.id}/edit`} class="btn btn-accent">Edit preset</a>
	{/if}
...
{#each data.presetData as presetDataEntry, i}
	...
	<div class="break-words p-4 bg-base-200">
		<code class="font-mono">{@html presetDataEntry.data.replace(/[\r\n]+/g, '<br />')}</code>
	</div>
{/each}
